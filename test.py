...
class RollTestCase(TestCase):
    ...
    def test_disallows_too_many_components(self):
        ...
        self.assertRaises(TooManyComponentsException, lambda: Roll([dice], modifiers))

    def test_disallows_small_modifier(self):
        dice = Dice(1, 20)
        self.assertRaises(OutOfRangeException, lambda: Roll([dice], [0]))

    def test_disallows_large_modifier(self):
        dice = Dice(1, 20)
        modifier = Roll.MAX_MODIFIER + 1
        self.assertRaises(OutOfRangeException, lambda: Roll([dice], [modifier]))
...
class RollCommandTestCase(TestCase):
    ...
    def test_invalid_first_argument(self):
        self.assertRaises(
            InvalidSyntaxException, lambda: RollCommand.from_args(["adv"])
        )

    def test_disallows_too_many_rolls(self):
        roll = Roll([Dice(1, 20)], [])
        rolls = [roll] * (Roll.MAX_COMPONENTS + 1)
        self.assertRaises(TooManyComponentsException, lambda: RollCommand(rolls))
...
class SavedRollManagerTestCase(TestCase):
    def setUp(self):
        ...
        cursor.execute(
            self.srm.sql["save"],
            {"name": "example_roll", "args": "1d20 adv", "user": 12345},
        )
        connection.commit()
    ...
    def test_invalid_roll(self):
        self.assertRaises(
            InvalidSyntaxException, lambda: self.srm.save("test_roll", ["adv"], 12345)
        )
    ...
    def test_get_throws_on_nonexistent_roll(self):
        self.assertRaises(DoesNotExistException, lambda: self.srm.get("nothing", 12345))
    ...
    def test_delete_throws_on_nonexistent_roll(self):
        self.assertRaises(
            DoesNotExistException, lambda: self.srm.delete("nothing", 12345)
        )
